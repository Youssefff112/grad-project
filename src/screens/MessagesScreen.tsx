    ...
    <SafeAreaView style={[tw`flex-1`, { backgroundColor: isDark ? '#0a0a12' : '#f8f7f5' }]}>
      ...
      <ScrollView style={tw`flex-1`} contentContainerStyle={tw`pb-24`}>
        {CONVERSATIONS.map((convo, index) => (
          <TouchableOpacity
            key={convo.id}
            onPress={() => navigation.navigate('Chat', { chatName: convo.name, isAI: convo.isAI })}
            style={[
              tw`flex-row items-center gap-4 px-4 py-4`,
              index < CONVERSATIONS.length - 1 && { borderBottomWidth: 1, borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)' },
            ]}
          >
            <View style={[tw`w-12 h-12 rounded-full items-center justify-center`, { backgroundColor: convo.isAI ? accent + '20' : isDark ? '#1e293b' : '#f1f5f9', borderWidth: convo.isAI ? 1 : 0, borderColor: accent + '40' }]}>
              <MaterialIcons name={convo.avatar as any} size={24} color={convo.isAI ? accent : isDark ? '#94a3b8' : '#64748b'} />
            </View>
            <View style={tw`flex-1`}>
              <View style={tw`flex-row items-center justify-between mb-1`}>
                <View style={tw`flex-row items-center gap-2`}>
                  <Text style={[tw`text-base font-bold`, { color: isDark ? '#f1f5f9' : '#1e293b' }]}>{convo.name}</Text>
                  {convo.isAI && (
                ...
                <Text style={[tw`text-xs`, { color: '#94a3b8' }]}>{convo.time}</Text>
              </View>
              <View style={tw`flex-row items-center justify-between`}>
                <Text style={[tw`text-sm flex-1 mr-3`, { color: convo.unread > 0 ? (isDark ? '#cbd5e1' : '#475569') : '#94a3b8' }]} numberOfLines={1}>
                  {convo.lastMessage}
                </Text>
                {convo.unread > 0 && (
